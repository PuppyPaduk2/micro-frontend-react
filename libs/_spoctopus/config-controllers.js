...
const createConfigController = (payload = {}) => {
  const pathPackageDir = $path.resolve(payload.packageDir || process.cwd());
  const pathPackageJson = $path.resolve(pathPackageDir, PATHS.PACKAGE_JSON);
  ...
  const controller = {
    ...
    read: () => {
      pack = $fs.readJsonSync(pathPackageJson);
    ...
    write: () => {
      ...
      $fs.writeJsonSync(pathPackageJson, pack);
      return controller;
    },
    takeLink: (payload) => {
